...
from MergeData import process_data
import time
...
class GraphMaker(object):
    ...
    def delete_all_node_relation(self):
        self.g.delete_all()
...
if __name__ == '__main__':
    build_neo4j_flag = False
    graph_maker = GraphMaker()
    diseases_dict = process_data(['./medical_data/medical.json'])
    data = graph_maker.extract_data(diseases_dict)
    if build_neo4j_flag:
        is_delete_node_relation = False
        start_time = time.time()
        # fixme 清除之前的所有数据集,慎用
        if is_delete_node_relation:
            graph_maker.delete_all_node_relation()

        graph_maker.make_nodes(data)
        graph_maker.make_rels(data)
        print(f"插入数据用时{time.time()-start_time}秒")
    # output the data in the form of jieba dict, it's for jieba
    output_data(data['depts'], '../entity_dict/Departments_dic.txt', True)
    output_data(data['drugs'], '../entity_dict/Drugs_dic.txt', True)
    output_data(data['symptoms'], '../entity_dict/Symptoms_dic.txt', True)
    output_data(data['foods'], '../entity_dict/Foods_dic.txt', True)
    output_data(data['disease_names'], '../entity_dict/Diseases_dic.txt', True)
